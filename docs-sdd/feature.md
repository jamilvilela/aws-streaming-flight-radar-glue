...
| `cod_unico` | `string` | PK concatenation for merge |
...
Delta Lake specific configs are included: `spark.databricks.delta.properties.defaults.autoOptimize.optimizeWrite`, `spark.databricks.delta.properties.defaults.autoOptimize.autoCompact`, plus the Delta Spark extension and catalog (`spark.sql.extensions`, `spark.sql.catalog.spark_catalog`).
...
flowchart TD
    ...
    REJ["4. Writer.write_rejects(rejects_df, target)"]
...
| 6 | Rejected records are written to the `Rejected/` location | ☐ |
| 7 | Job writes validated data in Delta Lake to the raw bucket | ☐ |
| 8 | Data is partitioned by `event_date` | ☐ |
